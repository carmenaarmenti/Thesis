...
By doing so, we clearly obtained 4 level of difficulty; so we defined an incremental difficulty criteria to feed the model with: 
the bug-fixing training scheduler described as follows. Not only did we split the training dataset in 4 sub-datasets, but 
...
steps set in the beginning, early stopping is run and we set a patience of 5 checkpoint. For 
patience we mean the number of steps with no improvement after which training will be stopped. We set 
...
were excluded from the set. The same happend in the case of the evaluation set, 
...
the source methods.\\
It must be said that from the initial training and evaluation datasets were removed instances whose
...
\noindent\textbf{Training Scheduler.} Once again we observed the data distribution and considered the quartiles
